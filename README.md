...
### ⚙️ 技术特性
- **多模型架构** - 支持Qwen3-VL-2B/4B/8B的Instruct、Thinking、FP8、AWQ等12个模型变体
- **智能设备管理** - 自动检测CUDA可用性，支持CPU/GPU无缝切换
- **高级量化系统** - 支持无量化(FP16)、4位/8位量化，平衡性能与内存使用
...
2. 克隆此仓库：
   ```bash
   git clone https://github.com/otsluo/ComfyUI-QwenVL-All.git
   ```
3. 安装依赖：
   ```bash
...
**最新优化**：
- 扩展模型支持：新增Qwen3-VL-2B/4B/8B的Thinking、FP8、AWQ等12个模型变体
...
- **识别速度优化**：如果图像/视频识别时间过长，建议降低输入尺寸以提升处理速度
...
- ✅ 检查网络连接状态
- ✅ 确认磁盘空间充足（每个模型2-8GB）
- ✅ 手动下载：将模型文件放入 `ComfyUI/models/LLM/Qwen-VL/` 目录
...
**FlashAttention2相关错误**
- ✅ 问题描述：选择"Flash Attention 2: 最佳性能"时出现"package flash_attn seems to be not installed"错误
- ✅ 解决方案：安装flash-attn包
  1. 使用pip安装：`pip install flash-attn --no-build-isolation`
  2. 如果遇到编译问题，请参考项目根目录的 [FLASH_ATTENTION_INSTALLATION.md](FLASH_ATTENTION_INSTALLATION.md) 文件获取详细安装指南
  3. 替代方案：选择"SDPA: 平衡"或"Eager: 最佳兼容性"作为注意力机制类型
